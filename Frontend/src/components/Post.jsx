...
const Post = ({ post }) => {
  return (
    <div className="flex flex-col lg:flex-row rounded-3xl shadow-2xl min-w-[400px]">
      ...
      <div className="px-4 lg:px-8 py-4 w-full lg:w-1/2">
        <div className="header flex justify-between items-center">
          ...
          <div className="category font-mono">
            <span className="text-[#1B4CD2] border-gray-50 rounded-xl px-4 py-1">{"// "} {post.category.name}</span>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2 my-4">
          {post.tags.map(tag => <span key={tag} className="px-2 text-sm bg-[#1B4CD2] rounded-xl">{tag}</span>)}
        </div>
